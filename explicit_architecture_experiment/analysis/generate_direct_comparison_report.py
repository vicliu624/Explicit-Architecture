...
# -*- coding: utf-8 -*-
"""
generate_direct_comparison_report.py
----------------------------------------------------
直接对比分析报告生成脚本
...
def load_predictions(predictions_file):
    ...
    if not os.path.exists(predictions_file):
        print(f"⚠️ 未找到预测文件: {predictions_file}")
        return []
...
def analyze_coupling_difference(mvc_coupling, explicit_coupling):
    ...
    # 计算平均耦合度
    mvc_avg = mvc_coupling[['import_coupling', 'call_coupling', 'coupling_score']].mean()
    explicit_avg = explicit_coupling[['import_coupling', 'call_coupling', 'coupling_score']].mean()
...
def create_comparison_plots(mvc_predictions, explicit_predictions, mvc_coupling, explicit_coupling, output_dir):
    """创建对比图表"""
    ...
    # 性能指标对比
    mvc_metrics = calculate_metrics(mvc_predictions)
    explicit_metrics = calculate_metrics(explicit_predictions)
...
def generate_report(mvc_predictions_file, explicit_predictions_file, mvc_coupling_file, explicit_coupling_file, output_file, output_dir):
    """生成完整的对比分析报告"""
    
    print("📊 加载预测结果...")
    mvc_predictions = load_predictions(mvc_predictions_file)
    explicit_predictions = load_predictions(explicit_predictions_file)
    ...
    print("📊 生成对比图表...")
    create_comparison_plots(mvc_predictions, explicit_predictions, mvc_coupling, explicit_coupling, output_dir)
    ...
    report = {
        ...
        'conclusions': {
            ...
            'coupling': '显性架构具有更低的耦合度' if coupling_analysis and coupling_analysis['explicit_averages'].get('coupling_score', 0) < coupling_analysis['mvc_averages'].get('coupling_score', 0) else 'MVC架构具有更低的耦合度'
        }
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="生成MVC架构 vs 显性架构直接对比分析报告")
    parser.add_argument("--mvc_predictions", required=True, help="MVC架构预测结果文件")
    parser.add_argument("--explicit_predictions", required=True, help="显性架构预测结果文件")
    ...
    generate_report(
        args.mvc_predictions,
        args.explicit_predictions,
        args.mvc_coupling,
